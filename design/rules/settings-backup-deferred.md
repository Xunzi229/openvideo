...
> **状态：导出 / 导入基础流程已实现，设置页入口暂时关闭**
> **开关：** `SettingsBackupUiPolicy.SETTINGS_EXPORT_ENTRY_VISIBLE = false`；`SettingsBackupUiPolicy.SETTINGS_IMPORT_ENTRY_VISIBLE = false`
...
- **0.3.3**（SAF 导出 JSON 到 `openvideo-settings.json`）底层已实现，设置页入口暂时关闭。
- **0.3.4 MVP**（SAF 导入 JSON）底层已实现，设置页入口暂时关闭；坏 JSON / 不支持版本会 Toast 降级，不写入设置。
...
| 0.3.5 设置页入口抛光 | 当前入口关闭；后续可替换为 Web / Hybrid 流程后再开放 |
...
| 2026-05-28 | 设置页备份导入 / 导出入口暂时关闭；底层导入导出能力保留 |
