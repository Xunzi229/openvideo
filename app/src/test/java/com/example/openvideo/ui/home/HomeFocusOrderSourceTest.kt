...
class HomeFocusOrderSourceTest {
    ...
    fun homeDynamicFilterChipsHaveExplicitDpadOrder() {
        ...
        assertTrue(source.contains("private fun dynamicChipChildren(group: LinearLayout): List<TextView>"))
        assertTrue(source.contains("if (chip.id == View.NO_ID) chip.id = View.generateViewId()"))
        assertTrue(source.contains("private fun wireDynamicChipRow(chips: List<TextView>, nextUpId: Int, nextDownId: Int)"))
