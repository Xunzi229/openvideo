# OpenVideo

**[English](README.md)**

OpenVideo 是一款使用 Kotlin 开发的轻量、易用的 Android 视频播放器。面向较新的 Android 版本，采用 Media3（ExoPlayer）作为播放内核，提供可自定义的播放界面、播放列表、播放历史与本地设置等功能。

本项目主要希望解决以下问题：
1. 市面上的许多播放器功能臃肿，或包含较多广告。
2. OpenVideo 希望提供一个轻便、简洁、实用的替代选择。

> [!NOTE]
> 本项目基本由 AI 辅助完成，不保证代码质量、稳定性与兼容性，请勿在重要场景中未经评估直接使用。如遇 Bug，欢迎提交 Issue；我会在个人时间允许的情况下尽力排查并修复，但无法保证具体处理时效。

## 环境要求

- 支持 AGP 9 的 Android Studio，或直接使用仓库内置 Gradle Wrapper
- JDK 17
- Gradle **9.5**，Android Gradle Plugin **9.0.1**
- Kotlin **2.2.10**，KSP **2.3.7**
- Android SDK：**compileSdk 35**，**minSdk 23**

项目使用 AGP 9 内置 Kotlin 支持。KSP 生成源码由常规 debug、单元测试与发布构建流程验证。

## 编译

在项目根目录执行：

```bash
./gradlew :app:assembleDebug
```

Windows：

```bat
gradlew.bat :app:assembleDebug
```

Release 构建会启用 R8 压缩；发布前请自行配置签名。

## 目录结构（概览）

| 路径 | 说明 |
|------|------|
| `app/src/main/java/.../ui/` | 界面：Activity、Fragment、播放器 UI |
| `app/src/main/java/.../core/` | 播放、偏好设置、数据库等 |
| `gradle/libs.versions.toml` | 依赖版本目录 |

第三方开源组件的许可信息在构建时由 Gradle 插件汇总，可在应用内 **设置 → 开源许可** 中查看。

## 界面设计（参考稿）

`design/` 目录下的静态稿用于说明播放器遮罩层的视觉风格（顶栏/底栏、半透明圆形按钮、蓝色进度条等）。以下为横屏与竖屏参考图，不代表运行时截图。

| 横屏 | 竖屏 |
|------|------|
| ![横屏播放界面参考](design/横屏播放界面.png) | ![竖屏播放界面参考](design/竖屏播放界面.png) |

## 参与贡献

欢迎提交 Issue 与 Pull Request；请尽量保持改动范围清晰，并遵循现有代码风格。

如果你有相关开发经验，并愿意共同维护 OpenVideo，欢迎通过本仓库的 Issue 与我联系。

## 许可证

OpenVideo 使用 MIT License 发布；各依赖库的具体许可亦可在应用内 **开源许可** 页面查看。
